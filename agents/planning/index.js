...
class PlanningAgent {
  ...
  async initialize() {
    ...
    this.eventBus.subscribe(
      EventBus.Events.LINEAR_PROJECT_CREATED,
      this._handleLinearProjectCreated.bind(this),
      "planning",
    );
    this.eventBus.subscribe(
      EventBus.Events.GITHUB_REPO_CREATED,
      this._handleGithubRepoCreated.bind(this),
      "planning",
    );
  ...
  async _distributeTasks(tasks) {
    ...
    for (const [agent, tasks] of Object.entries(agentTasks)) {
      ...
      try {
        ...
        console.log(
          `[PlanningAgent] Assigned ${tasks.length} tasks to ${agent}`,
        );
      } catch (error) {
        console.error(
          `[PlanningAgent] Failed to assign tasks to ${agent}:`,
          error,
        );
      }
    }
  ...
  async _handleLinearProjectCreated(event) {
    console.log(
      "[PlanningAgent] New Linear project detected, triggering analysis",
    );
  ...
  async _handleGithubRepoCreated(event) {
    console.log(
      "[PlanningAgent] New GitHub repo detected, triggering analysis",
    );
  ...
  getStatus() {
    return {
      ...
      pendingTasks: this.taskGenerator.taskQueue.filter(
