...
async function activateTITAN() {
  ...
  for (const system of systems) {
    try {
      ...
      if (response.status === 200) {
        ...
        console.log(
          chalk.red(`❌ ${system.name.padEnd(25)} FAILED (${response.status})`),
        );
        failedCount++;
      }
    } catch (error) {
  ...
  console.log(chalk.cyan(`\n📊 ACTIVATION SUMMARY:\n`));
  console.log(
    chalk.green(`✅ Active Systems:  ${activeCount}/${systems.length}`),
  );
  console.log(
    chalk.red(`❌ Failed Systems:  ${failedCount}/${systems.length}`),
  );
