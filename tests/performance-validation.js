...
test("Event Bus - Efficient array trimming", () => {
  ...
  assert.strictEqual(
    eventBus.eventLog.length,
    maxSize,
    "Event log should be trimmed to max size",
  );

  // Verify oldest events were removed (should start at index 100)
  const oldestEvent = eventBus.eventLog[0];
  assert.strictEqual(
    oldestEvent.data.data,
    100,
    "Oldest events should be removed first",
  );
});
...
test("Analytics - O(1) average calculation", () => {
  ...
  assert.ok(
    Analytics.metrics.performance.runningSum > 0,
    "Running sum should be maintained",
  );
  ...
  assert.ok(
    ...
    `Average should be close to ${expectedAvg}, got ${actualAvg}`,
  );
...
test("Immutable Logger - Efficient append operations", () => {
  ...
  assert.ok(
    chmodCallCount < 10,
    `chmod should not be called 3x per log (was called ${chmodCallCount} times for 10 logs)`,
  );
...
test("Parallel execution with Promise.all", async () => {
  ...
  assert.ok(
    duration < 200,
    `Parallel execution should take ~100ms, took ${duration}ms`,
  );
...
test("Window function concept - single-pass counting", () => {
  ...
  assert.ok(
    !cleanedRows[0].hasOwnProperty("total_count"),
    "total_count should be removed from results",
  );
});
